...
throw new Error("Module build failed: Error: ENOENT: no such file or directory, open 'C:\\Users\\Кампутер\\Desktop\\Lesson 15\\node_modules\\formdata-polyfill\\formdata.min.js'");
...
function calc() {
  ...
  var persons = document.querySelectorAll('.counter-block-input')[0],
      ...
      daysSum = 0,
      total = 0;
  totalValue.innerHTML = 0; // Сбрасывает цифры
  ...
  function numbers(event) {
    var No = event.keyCode;

    if (No == 46 || No == 8 || No == 9 || No == 27 || No == 65 && event.ctrlKey === true || No >= 35 && No <= 39) {
      return;
    } else {
...
function tabs() {
  ...
  var hideTabContent = function hideTabContent(a) {
    tabContent.forEach(function (item, key) {
      if (key !== a) {
...
function timer() {
  ...
  function setClock(id, endtime) {
    ...
    function updateClock() {
      ...
      if (t.total <= 0) {
        clearInterval(timeInterval); // Когда закончится таймер он не пойдет в минус
